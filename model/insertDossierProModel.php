...
class insertDossierProModel extends Bdd {
    ...
    public function insertT7($table7) {
        $sql = $this->getBdd()->prepare("INSERT INTO table7(diplome, organisme, date, id_user) VALUES :dipl, :orga, :date, :id");
        $sql->bindParam(':dipl', $table7[0]);
        $sql->bindParam(':orga', $table7[1]);
        $sql->bindParam(':date', $table7[2]);
        $sql->bindParam(':chant', $_SESSION['id_user']);
        $sql->execute();
    }
