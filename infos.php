...
// function connection base getBdd()
  public function setIdNom(){

    $sql = $this->getBdd()->prepare('INSERT INTO dossiers (ID_P, Prenom, Nom_N_proprio, Nom_U_Proprio, Adresse) VALUES (:id, :prenom, :nom_n, :nom_u, :addr)');
  ...
  public function getIdNom(){
    $sql = $this->getBdd()->prepare('SELECT id FROM table WHERE nomnaissance = :nomnaissance AND prenom = :prenom');
  ...
  public function addEntry(){
    ...
    $tout = json_encode($this->values);
     $sql = $this->getBdd()->prepare('INSERT INTO dossier (Nom_F) VALUE (:nom_f) WHERE id = (SELECT id FROM dossier WHERE nomusage = :nomusage)');
